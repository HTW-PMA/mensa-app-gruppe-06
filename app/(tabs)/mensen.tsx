...
export default function MensenScreen() {
  ...
  useFocusEffect(
  useCallback(() => {
    loadFavorites(); // 🔄 Reload favorites when screen regains focus
  }, [])
);
    ...
    return (
      ...
      >
        <View style={styles.row}>
          <Text style={[styles.name,{width: '85%'}]}>{item.name}</Text>
          <TouchableOpacity onPress={() => toggleFavorite(item.id)} style={{width:'10%'}}>
            <Ionicons
